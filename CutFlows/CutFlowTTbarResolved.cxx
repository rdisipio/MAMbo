...
bool CutFlowTTbarResolved::Initialize() {
        ...
        m_isMCSignal = m_config->custom_params_flag["isMCSignal"];
        m_isRealData = m_config->custom_params_flag["isRealData"];
        m_isWjets    = m_config->custom_params_flag["isWjets"];
        m_isQCD      = m_config->custom_params_flag["isQCD"];        

        m_decay = m_config->custom_params_string["decay"];
        //  int isDilepton = (int) m_config->custom_params_flag["isDilepton"];
        cout << "INFO: signal flags: " << " m_isMCSignal=" << m_isMCSignal << " m_decay=" << m_decay << endl;
...
*/
        ...
        SetCutName("LPLUSJETS", "reco_unweight", 0, "All AnalysisTop Events");
        SetCutName("LPLUSJETS", "reco_unweight", 1, "Nlep == 1 && NJets >= 2");
        SetCutName("LPLUSJETS", "reco_unweight", 2, "NJets >= 3");
        SetCutName("LPLUSJETS", "reco_unweight", 3, "NJets >= 4");
        SetCutName("LPLUSJETS", "reco_unweight", 4, "NbJets >= 1");
        SetCutName("LPLUSJETS", "reco_unweight", 5, "NbJets >= 2");


        AddCounterName("LPLUSJETS", "reco_weighted", 5 );
        SetCutName("LPLUSJETS", "reco_weighted", 0, "All AnalysisTop Events");
        SetCutName("LPLUSJETS", "reco_weighted", 1, "Nlep == 1 && NJets >= 2");
        SetCutName("LPLUSJETS", "reco_weighted", 2, "NJets >= 3");
        SetCutName("LPLUSJETS", "reco_weighted", 3, "NJets >= 4");
        SetCutName("LPLUSJETS", "reco_weighted", 4, "NbJets >= 1");
        SetCutName("LPLUSJETS", "reco_weighted", 5, "NbJets >= 2");    



        if( m_isRealData || m_isWjets || m_isQCD ) return success;
        ...
        m_isStressTest = 0;
        m_stressTestType = "none";

        if( m_config->custom_params_string.count( "stressTest" ) ) //mr
        {
                m_stressTestType = m_config->custom_params_string["stressTest"];
                if( m_stressTestType != "none" && m_stressTestType != "tt_pt" && m_stressTestType != "tt_m" && m_stressTestType != "tt_rapidity" && m_stressTestType != "t_pt" )
                {
                        cout << "Warning: stress test type " << m_stressTestType << " is unknown, setting it to \"none\"\n";
                        m_stressTestType = "none";
                }
                else if(  m_stressTestType != "none" )
                {
                        m_isStressTest = 1;
                        
                }
                cout << "Stress test type = " << m_stressTestType << endl;
        }    
...
bool CutFlowTTbarResolved::Apply(EventData * ed) {
        ...
        if( !m_isRealData && !m_isQCD ) {
                weight_reco_level     = ed->info.mcWeight;
                weight_particle_level = ed->info.mcWeight;

                if( m_isStressTest )//mr
                {
                        if( m_stressTestType == "" )
                        {
                                char msg[100];
                                sprintf( msg, "IsStress test is true, but m_stressTestType is empty\n");
                                throw runtime_error( msg );
                        }
                        if( ed->info.eventNumber % 1000 == 0 )
                        {
                                cout << "Event " << ed->info.eventNumber << ", stress test " << m_stressTestType << endl;
                        }        
                        
                        TLorentzVector t1 = Particle(ed->mctruth, 0).MakeLorentz();
                        TLorentzVector t2 = Particle(ed->mctruth, 1).MakeLorentz();
                        TLorentzVector tt =  Particle(ed->mctruth, 2).MakeLorentz();
                        double stressFactor = 1;
                        if( m_stressTestType == "tt_rapidity" )
                        {
                                //Rapidity gaussian reweight
                                double sigma = 0.3;
                                double ratio2= tt.Rapidity() * tt.Rapidity() / (sigma*sigma);
                                stressFactor = 1 - 0.4* TMath::Exp( -1 * ratio2 );
                                
                        }
                        else if( m_stressTestType == "tt_m" )
                        {
                                //         Mass bump"
                        ...
                        else if( m_stressTestType == "tt_pt" )
                        {
                                // tt pt slope
                                stressFactor = 1 + tt.Pt() / 600000;

                        }
                        else
                        {
                                //top pt slope
                                stressFactor = 1 + ( t1.Pt() + t2.Pt())/ 1500000; //average of the pt

                        }
                        weight_particle_level *= stressFactor;
                        weight_reco_level *= stressFactor;
                }
...
*/
        ...
        const bool passedParticleSelection = ( m_isRealData || m_isWjets || m_isQCD ) ? false : PassedCutFlowParticle( ed );


        weight_particle_level = ed->property["weight_particle_level"];
        ...
        if( m_isMCSignal ) {
                if( ( m_decay == "ljets" ) && ( EventIsDileptonic == 1 ) ) return success;
        }
        ...
        if (m_isMCSignal and splitSample) {
        ...
        if( !m_isMCSignal ) return success;

        //    if( (!m_isMCSignal) || (isDilepton==1) )
        //      return success;
...
bool CutFlowTTbarResolved::Finalize()
{
        ...
        /*
unsigned long m_isQCD = m_config->custom_params_flag["m_isQCD"];
if( !m_isQCD ) return success;
*/
...
bool CutFlowTTbarResolved::PassedCutFlowParticle(EventData * ed) {    
        ...
        //1     //****************All event passing analysis top selection *************************
        const bool passed_resolved_ejets = ed->property["passed_particle_resolved_ejets_2j0b"]; ///Preselection done in AT for el-channel
        const bool passed_resolved_mujets = ed->property["passed_particle_resolved_mujets_2j0b"]; ///Preselection done in AT for mu-channel  
        const bool  analysistop_cutflow = ( m_config->channel == kElectron )  ?  ( passed_resolved_ejets) : ( passed_resolved_mujets );     
        if( !analysistop_cutflow )   return !passed;
        PassedCut( "LPLUSJETS", "particle_unweight" );
        PassedCut( "LPLUSJETS", "particle_weighted", weight );
...
void CutFlowTTbarResolved::FillHistogramsPseudotopResponseRecoToParticle(    EventData * ed, const double weight )
{
        ...
        m_hm->FillMatrices("reco/4j2b/difference/Matrix_reco_particle_absPout",m_VarField.find("reco_absPout")->second / GeV, m_VarField.find("particle_absPout")->second / GeV, weight);
        ...
        m_hm->FillMatrices("particle/4j2b/difference/Matrix_particle_reco_absPout",m_VarField.find("particle_absPout")->second / GeV, m_VarField.find("reco_absPout")->second / GeV, weight);
...
void CutFlowTTbarResolved::FillHistogramsPseudotopResponseRecoToParton(      EventData * ed, const double weight )
{
        ...
        m_hm->FillMatrices("reco/4j2b/difference/Matrix_reco_parton_absPout",m_VarField.find("reco_absPout")->second / GeV, m_VarField.find("parton_absPout")->second / GeV, weight);
        ...
        m_hm->FillMatrices("parton/4j2b/difference/Matrix_parton_reco_absPout",m_VarField.find("parton_absPout")->second / GeV, m_VarField.find("reco_absPout")->second / GeV, weight);
