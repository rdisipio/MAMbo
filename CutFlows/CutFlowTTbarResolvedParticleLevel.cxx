...
bool CutFlowTTbarResolvedParticleLevel::Apply( EventData * ed )
{
        ...
        m_EventIsDileptonic = ed->property["isDileptonic"];
        //cout << "DEBUG: EventIsDileptonic = " << EventIsDileptonic << endl;
        // RDS+MR
        if( m_isMCSignal ) 
        {
                
                if( m_decay == "ljets" && m_EventIsDileptonic != 0 ) return success;
        }
    /*    if( ed->info.eventNumber % 1000 == 0 )
        {
                cout << "Event " << ed->info.eventNumber << ", do stress test " << m_isStressTest << endl;
        }
*/

        if( m_isStressTest )//mr
        {
                //m_stressTestType = m_config->custom_params_string["stressTest"];//tmp fix
                if( m_stressTestType == "" )
                {
                        char msg[100];
   ...
   */             
                 ...
                 if( m_stressTestType == "tt_rapidity" )
                {
                        ...
                        weight_particle_level *= 1 - 0.4* TMath::Exp( -1 * ratio2 );
                }
              ...
              //          if( delta < 100000 ) cout << "delta = " << delta << " sigma " << sigma << " mass " << tt.M() << " exp factor " << 2*TMath::Exp( -1 *  ratio2 )<< endl;

                        weight_particle_level *= 1 + 2*TMath::Exp( -1 * ratio2 );
                }
