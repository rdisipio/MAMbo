...
int main( int nargs, char **args )
{
  ...
  // gROOT->LoadMacro("AtlasUtils.C");
	...
	int nbins;
	string reco_histoPath =  "reco/1fj1b/" + particle + "/" + variable ;
	string particle_histoPath =  "particle/1fj1b/" + particle + "/" + variable;
	string response_histoPath ="reco/1fj1b/" + particle + "/Matrix_reco_particle_" + variable;
	char histoname[20];
	for( int i = 0; i <= 30; ++i )
	{
		char temp[1000];
		sprintf( temp, "output/PDF_all/sum_PDF4LHC15_nlo_30_%i_%s.root", i, "nominal");
	...
	sprintf( temp, "output/PDF_all/sum_PDF4LHC15_nlo_30_%i_%s.root", i, "nocut");
		cout << "Opening " << temp << endl;
	...
	string reco_ct10path = "output/PDF_all/sum_tt_diffxs_13TeV.mc.AFII.DiTop.410003.nominal.ljets.histograms.root";
	string particle_ct10path = "output/PDF_all/sum_tt_diffxs_13TeV.mc.AFII.DiTop.410003.nocut.ljets.histograms.root";
	...
	legend->Draw();
	c->Print( ( "plots/eff_" + particle + "_" + variable + ".png" ).c_str() );
	...
	legend->Draw();
	c->Print( ( "plots/acc_" + particle + "_" + variable + ".png" ).c_str() );	
